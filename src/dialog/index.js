...
const global = define('dialog', { template, props, setup })

global.dialog = (title = '', option = {}) => {
  const dom = document.createElement('m-dialog')
  dom.title = title
  dom.message = option.message || ''
  dom.positive = option.positive || ''
  dom.negative = option.negative || ''
  let onPositive = option.onPositive || null
  let onNegative = option.onNegative || null
  let onClose = option.onClose || null
