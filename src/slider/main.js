...
const template = `<style>:host{display:block;position:relative;height:18px;user-select:none}:host([disabled=true]){pointer-events:none;filter:grayscale(1);opacity:.6}input{height:100%;width:100%;margin:0;cursor:pointer;opacity:1;position:absolute}input:active+div .bar,.bar.ripple{transform:scale(1.5);box-shadow:0 0 0 6px rgba(var(--color-accent),.24)}div{height:100%;display:flex;align-items:center;position:relative;pointer-events:none;margin:0 6px;width:calc(100% - 12px)}div::before{content:'';display:block;height:2px;width:100%;background:rgba(var(--color-accent),.24)}span{position:absolute}.line{height:2px;display:block;background:rgba(var(--color-accent));width:50%}.bar{display:block;width:12px;height:12px;border-radius:50%;background:rgba(var(--color-accent));margin-left:-6px;transition:transform .2s,box-shadow .2s;left:50%}</style><input type="range" max="100" value="50" step="1"><div><span class="line"></span><span class="bar"></span></div>`
